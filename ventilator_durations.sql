...
DROP MATERIALIZED VIEW IF EXISTS ventilator_durations; 
CREATE MATERIALIZED VIEW ventilator_durations AS
WITH vs AS
(
    select
    stay_id, charttime
    ...
    , MAX(CASE
        ...
        WHEN ventilator_mode IS NOT NULL OR ventilator_mode_hamilton IS NOT NULL THEN 1
        WHEN minute_volume IS NOT NULL THEN 1
        WHEN tidal_volume_observed IS NOT NULL THEN 1
        WHEN tidal_volume_set IS NOT NULL THEN 1
...
(
  select
      ...
      , (charttime - charttime_lag)/60 as ventduration
...
select stay_id
  ...
  , max(charttime) as endtime
	,date_part('epoch'::text, max(charttime) - min(charttime)) / 60::double precision / 60::double precision AS duration_hours

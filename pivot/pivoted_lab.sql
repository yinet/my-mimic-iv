  ...
  select
    ...
    , case
        ...
        and i.outtime_lag > (i.intime-INTERVAL '24' HOUR)
          then (i.intime-INTERVAL CAST(DATETIME_DIFF(i.intime, i.outtime_lag, SECOND)/2 AS INT64) SECOND)
      else (i.intime-INTERVAL '12' HOUR)
      end as data_start
    , case
        when i.intime_lead is not null
        and i.intime_lead < (i.outtime+ INTERVAL '24' HOUR)
          then (i.outtime+INTERVAL CAST(DATETIME_DIFF(i.intime_lead, i.outtime, SECOND)/2 AS INT64) SECOND)
  ...
  select
    ...
    , case
        ...
        and h.dischtime_lag > (h.admittime-INTERVAL '24' HOUR)
          then (h.admittime-INTERVAL CAST(DATETIME_DIFF(h.admittime, h.dischtime_lag, SECOND)/2 AS INT64) SECOND)
      else (h.admittime- INTERVAL '12' HOUR)
      end as data_start
    , case
        when h.admittime_lead is not null
        and h.admittime_lead < (h.dischtime+INTERVAL '24' HOUR))
          then (h.dischtime-INTERVAL CAST(DATETIME_DIFF(h.admittime_lead, h.dischtime, SECOND)/2 AS INT64) SECOND)
